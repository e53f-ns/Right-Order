...
function resolveJwtSecret(): string {
  const configuredSecret = process.env['JWT_SECRET'] ?? process.env['JWT_ACCESS_SECRET'];
  if (configuredSecret && configuredSecret.length >= 32 && !configuredSecret.includes('CHANGE_ME')) {
    return configuredSecret;
  }
  if (IS_PROD) {
    throw new Error('JWT_SECRET (or JWT_ACCESS_SECRET) must be set to a strong value (>=32 chars) in production');
  }
  const ephemeralSecret = crypto.randomBytes(32).toString('hex');
  logger.warn('JWT secret not configured securely in legacy auth module; using ephemeral dev-only secret');
...
function verifyPassword(password: string, storedHash: string): boolean {
  if (storedHash.startsWith('pbkdf2$')) {
    ...
    if (!salt || !expectedHash || !Number.isFinite(iterations) || iterations < 10_000) {
      return false;
    }
  ...
  // Legacy compatibility: previously used sha256(password + staticSalt)
  const legacy = crypto.createHash('sha256').update(password + 'ro-salt-v1').digest('hex');
  const expectedBuffer = Buffer.from(storedHash, 'utf8');
  const candidateBuffer = Buffer.from(legacy, 'utf8');
  if (expectedBuffer.length !== candidateBuffer.length) return false;
  return crypto.timingSafeEqual(expectedBuffer, candidateBuffer);
}
