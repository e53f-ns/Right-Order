...
import { z } from 'zod';
import { clampFloat, clampInt } from '../security/validators.js';
...
import {
  ...
  autoConfirmCryptoPaymentFromWebhook,
  stubUpgrade,
  getPaymentHistory,
  getAllPayments,
...
function classifyApiError(error: unknown, fallbackMessage: string): {
  status: number;
  error: string;
  code: string;
} {
  ...
  if (
    ...
    lower.includes('database') && lower.includes('connection') ||
    lower.includes('econnrefused') ||
    lower.includes('invalid `prisma.')
  ) {
    return {
      status: 503,
      error: 'Service temporarily unavailable (database connection issue). Please try again in a minute.',
      code: 'DB_UNAVAILABLE',
    };
  }
  ...
  if (lower.includes('does not exist') || lower.includes('relation') && lower.includes('does not exist')) {
    return {
      status: 503,
      error: 'Service database is not initialized. Please run database migrations.',
...
const checkoutSchema = z.object({
  plan: z.enum(['free', 'pro', 'elite', 'ultimate']),
  billing: z.enum(['monthly', 'yearly']),
});

const cryptoCheckoutSchema = checkoutSchema.extend({
...
export class DashboardServer {
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/api/dex/opportunities', (req: Request, res: Response) => {
      ...
      // Apply query filters
      const chainParam = typeof req.query['chain'] === 'string' ? req.query['chain'].trim().toLowerCase().slice(0, 32) : null;
      const dexParam = typeof req.query['dex'] === 'string' ? req.query['dex'].trim().toLowerCase().slice(0, 32) : null;
      const typeParam = typeof req.query['type'] === 'string' ? req.query['type'].trim().toLowerCase().slice(0, 32) : null;
      const minNet = clampFloat(req.query['minNet'], Number.NEGATIVE_INFINITY, -100, 10_000);

      if (chainParam) {
        dexSpreads = dexSpreads.filter(s => {
          const combined = `${s.buyExchange}|${s.sellExchange}`.toLowerCase();
          return combined.includes(chainParam);
        });
      }
      if (dexParam) {
        dexSpreads = dexSpreads.filter(s => {
          const combined = `${s.buyExchange}|${s.sellExchange}`.toLowerCase();
          return combined.includes(dexParam);
        });
      }
      if (typeParam) {
        ...
        dexSpreads = dexSpreads.filter(s => {
          ...
          return pathDesc.includes(typeParam);
        });
      }
      if (Number.isFinite(minNet)) {
    ...
    this.app.get('/api/messages/alerts', (req: Request, res: Response) => {
      ...
      const categorySchema = z.enum(['listing', 'pump', 'airdrop', 'hack', 'partnership', 'launch', 'whale', 'general']);
      const prioritySchema = z.enum(['high', 'medium', 'low']);
      const filter: { category?: z.infer<typeof categorySchema>; priority?: z.infer<typeof prioritySchema>; limit?: number } = {};

      if (typeof req.query['category'] === 'string') {
        const category = categorySchema.safeParse(req.query['category']);
        if (category.success) filter.category = category.data;
      }
      if (typeof req.query['priority'] === 'string') {
        const priority = prioritySchema.safeParse(req.query['priority']);
        if (priority.success) filter.priority = priority.data;
      }
      const parsedLimit = clampInt(req.query['limit'], 100, 1, 200);
    ...
    this.app.post('/api/nfts/scan', async (req: Request, res: Response) => {
      try {
        const nftScanSchema = z.object({
          chain: z.enum(['ethereum', 'polygon', 'solana']).optional(),
          minProfit: z.coerce.number().min(0).max(10_000).optional(),
        });
        const parsed = nftScanSchema.safeParse(req.body);
        if (!parsed.success) {
          res.status(400).json({ success: false, error: 'Invalid scan parameters', code: 'VALIDATION_ERROR', timestamp: nowMs() });
          return;
        }
        const nftChain = (parsed.data.chain ?? 'ethereum') as NFTChain;
        const minProfitPercent = parsed.data.minProfit ?? 10;
    ...
    this.app.get('/api/nfts/opportunities', (req: Request, res: Response) => {
      ...
      if (typeof chainParam === 'string' && chainParam) {
        const chain = chainSchema.safeParse(chainParam);
        if (chain.success) {
          filter.chain = chain.data as NFTChain;
        }
      }
      if (typeof req.query['minProfit'] === 'string') {
        filter.minProfit = clampFloat(req.query['minProfit'], 0, 0, 10_000);
      }
    ...
    this.app.post('/api/auth/register', async (req: Request, res: Response) => {
      try {
        ...
        logger.info(
          {
            email: parsed.success ? parsed.data.email.trim().toLowerCase() : undefined,
            passwordLength: parsed.success ? parsed.data.password.length : undefined,
            ip,
            ua,
          },
        ...
        if (!parsed.success) {
          const body = { success: false, error: 'Email and password required', code: 'VALIDATION_ERROR', timestamp: nowMs() };
          logger.warn({ status: 400, body, ip }, 'Register request rejected');
          res.status(400).json(body);
          return;
        }
        const result = await requestRegistrationCode(parsed.data.email, parsed.data.password, ip, ua);
        if (!result.success) {
          const status = result.error === 'Email already registered' ? 409 : 400;
          const code = result.error === 'Email already registered' ? 'EMAIL_TAKEN' : 'VALIDATION_ERROR';
          const body = { ...result, code, timestamp: nowMs() };
          logger.warn({ status, body, email: parsed.data.email.trim().toLowerCase(), ip }, 'Register request failed');
          res.status(status).json(body);
          return;
        }
        logger.info({ email: parsed.data.email.trim().toLowerCase(), ip }, 'Register code sent successfully');
        res.json({
          success: true,
          timestamp: nowMs(),
          requiresVerification: true,
          message: `Verification code sent to ${result.emailMasked ?? parsed.data.email.trim().toLowerCase()}`,
          expiresInSec: result.expiresInSec,
        });
      } catch (error) {
        const classified = classifyApiError(error, 'Registration failed');
        logger.error(
          {
        ...
        res.status(classified.status).json({ success: false, error: classified.error, code: classified.code, timestamp: nowMs() });
      }
    });

    this.app.post('/api/auth/register/verify', async (req: Request, res: Response) => {
      try {
        const parsed = verificationSchema.safeParse(req.body);
        if (!parsed.success) {
          res.status(400).json({ success: false, error: 'Email and code required', code: 'VALIDATION_ERROR', timestamp: nowMs() });
          return;
        }
        const ip = req.ip ?? req.socket.remoteAddress;
        const ua = req.headers['user-agent'];
        const result = await verifyRegistrationCode(parsed.data.email, parsed.data.code, ip, ua);
        if (!result.success) {
          const status = result.error === 'Email already registered' ? 409 : 400;
          res.status(status).json({ ...result, code: 'VERIFICATION_FAILED', timestamp: nowMs() });
          return;
        }
        ...
        const classified = classifyApiError(error, 'Verification failed');
        logger.error(
          { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined, code: classified.code },
    ...
    this.app.post('/api/auth/login', async (req: Request, res: Response) => {
      try {
        const parsed = authCredentialsSchema.safeParse(req.body);
        if (!parsed.success) {
          res.status(400).json({ success: false, error: 'Email and password required', code: 'VALIDATION_ERROR', timestamp: nowMs() });
          return;
        }
        const ip = req.ip ?? req.socket.remoteAddress;
        const ua = req.headers['user-agent'];
        const result = await requestLoginCode(parsed.data.email, parsed.data.password, ip, ua);
        if (!result.success) {
          res.status(401).json({ ...result, code: 'LOGIN_FAILED', timestamp: nowMs() });
          return;
        }
        res.json({
          success: true,
        ...
        const classified = classifyApiError(error, 'Login failed');
        logger.error({ error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined }, 'Login error');
        res.status(classified.status).json({ success: false, error: classified.error, code: classified.code, timestamp: nowMs() });
      }
    });

    this.app.post('/api/auth/login/verify', async (req: Request, res: Response) => {
      try {
        const parsed = verificationSchema.safeParse(req.body);
        if (!parsed.success) {
          res.status(400).json({ success: false, error: 'Email and code required', code: 'VALIDATION_ERROR', timestamp: nowMs() });
        ...
        const ip = req.ip ?? req.socket.remoteAddress;
        const ua = req.headers['user-agent'];
        const result = await verifyLoginCode(parsed.data.email, parsed.data.code, ip, ua);
        if (!result.success) {
          res.status(401).json({ ...result, code: 'VERIFICATION_FAILED', timestamp: nowMs() });
          return;
        }
        if (result.refreshToken) {
          res.cookie(REFRESH_COOKIE_NAME, result.refreshToken, REFRESH_COOKIE_OPTIONS);
        ...
        const classified = classifyApiError(error, 'Login verification failed');
        logger.error({ error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined }, 'Login verify error');
        res.status(classified.status).json({ success: false, error: classified.error, code: classified.code, timestamp: nowMs() });
      }
    });
    ...
    this.app.post('/api/payment/checkout', requireAuth, async (req: Request, res: Response) => {
      try {
        ...
        if (!parsed.success) {
          res.status(400).json({ success: false, error: 'plan and billing required', code: 'VALIDATION_ERROR', timestamp: nowMs() });
          return;
        }
        const idempotencyKey = getIdempotencyKey(req);
        const { plan, billing } = parsed.data;
        logger.info({ userId: req.user!.id, plan, billing, idempotencyKey }, 'Payment checkout request received');
        const configuredBaseUrl = process.env['PUBLIC_BASE_URL']?.trim();
        const baseUrl = configuredBaseUrl && /^https?:\/\/[a-zA-Z0-9.-]+(?::\d+)?$/.test(configuredBaseUrl)
          ? configuredBaseUrl
        ...
        const status = result.success
          ? 200
          : /not configured|temporarily unavailable|database/i.test(result.error ?? '') ? 503 : 400;
        res.status(status).json(result.success
          ? { ...result, timestamp: nowMs() }
          : { ...result, code: 'PAYMENT_CREATE_FAILED', timestamp: nowMs() });
      } catch (error) {
        const classified = classifyApiError(error, 'Payment initialization failed');
        logger.error(
          { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined, code: classified.code },
          'Stripe checkout route error'
        );
        res.status(classified.status).json({ success: false, error: classified.error, code: classified.code, timestamp: nowMs() });
      }
    });
    ...
    this.app.post('/api/payment/crypto', requireAuth, async (req: Request, res: Response) => {
      try {
        ...
        if (!parsed.success) {
          res.status(400).json({ success: false, error: 'plan, billing, network required', code: 'VALIDATION_ERROR', timestamp: nowMs() });
          return;
        }
        const { plan, billing, network } = parsed.data;
        const idempotencyKey = getIdempotencyKey(req);
        logger.info({ userId: req.user!.id, plan, billing, network, idempotencyKey }, 'Crypto payment request received');
        const result = await createCryptoPayment(req.user!.id, plan, billing, network, idempotencyKey);
        const status = result.success
          ? 200
          : /not configured|temporarily unavailable|database/i.test(result.error ?? '') ? 503 : 400;
        res.status(status).json(result.success
        ...
        const classified = classifyApiError(error, 'Crypto payment initialization failed');
        logger.error(
          { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined, code: classified.code },
          'Crypto payment route error'
        );
        res.status(classified.status).json({ success: false, error: classified.error, code: classified.code, timestamp: nowMs() });
      }
    });

    this.app.post('/api/payment/crypto/webhook', async (req: Request, res: Response) => {
      try {
        const expectedSecret = process.env['CRYPTO_WEBHOOK_SECRET'];
        if (!expectedSecret) {
          res.status(503).json({ success: false, error: 'Crypto webhook secret not configured', code: 'CRYPTO_WEBHOOK_NOT_CONFIGURED', timestamp: nowMs() });
          return;
        }

        const authHeader = req.headers['authorization'];
        const secretHeader = req.headers['x-crypto-webhook-secret'];
        ...
        if (!providedSecret || !safeSecretEquals(providedSecret, expectedSecret)) {
          logger.warn({ ip: req.ip }, 'Invalid crypto webhook secret');
          res.status(401).json({ success: false, error: 'Invalid webhook secret', code: 'INVALID_WEBHOOK_SECRET', timestamp: nowMs() });
          return;
        }

        const webhookSchema = z.object({
          txHash: z.string().trim().min(1).max(128).optional(),
          network: z.enum(['trc20', 'erc20', 'TRC20', 'ERC20']).optional(),
          amount: z.union([z.number(), z.string().trim().max(64)]).optional(),
          toAddress: z.string().trim().max(128).optional(),
          tokenSymbol: z.string().trim().max(16).optional(),
        ...
        if (!parsedPayload.success) {
          res.status(400).json({ success: false, error: 'Invalid crypto webhook payload', code: 'VALIDATION_ERROR', timestamp: nowMs() });
          return;
        }
        const payload = parsedPayload.data as CryptoWebhookPayload;
        logger.info(
          {
        ...
        const result = await autoConfirmCryptoPaymentFromWebhook(payload);
        const status = result.success ? 200 : 400;
        res.status(status).json({ ...result, code: result.success ? undefined : 'CRYPTO_AUTO_CONFIRM_FAILED', timestamp: nowMs() });
      } catch (error) {
        const classified = classifyApiError(error, 'Crypto webhook processing failed');
        logger.error(
          { error: error instanceof Error ? error.message : String(error), stack: error instanceof Error ? error.stack : undefined, code: classified.code },
          'Crypto webhook route error'
        );
        res.status(classified.status).json({ success: false, error: classified.error, code: classified.code, timestamp: nowMs() });
      }
    });
    ...
    this.app.post('/api/payment/crypto/confirm', requireAdmin, async (req: Request, res: Response) => {
      ...
      if (!parsed.success) { res.status(400).json({ success: false, error: 'paymentId and txHash required', code: 'VALIDATION_ERROR', timestamp: nowMs() }); return; }
    ...
    this.app.get('/api/payment/history', requireAuth, async (req: Request, res: Response) => {
      const parsed = paginationQuerySchema.safeParse({ page: req.query['page'], limit: req.query['limit'] });
    ...
    this.app.post('/api/admin/user/subscription', requireAdmin, async (req: Request, res: Response) => {
      ...
      if (!parsed.success) { res.status(400).json({ success: false, error: 'userId and plan required' }); return; }
    ...
    this.app.use((req: Request, res: Response) => {
      res.status(404).json({ success: false, error: 'Not found', code: 'NOT_FOUND', timestamp: nowMs() });
      logger.warn({ method: req.method, path: req.path }, 'Unknown route requested');
    });

    this.app.use((error: unknown, req: Request, res: Response, _next: express.NextFunction) => {
      const classified = classifyApiError(error, 'Internal server error');
      logger.error(
        {
          method: req.method,
          path: req.path,
